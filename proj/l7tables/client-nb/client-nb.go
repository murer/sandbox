...
func main() {
	...
	serverFD, err := unix.Socket(unix.AF_INET, unix.SOCK_STREAM|unix.SOCK_NONBLOCK|unix.SOCK_CLOEXEC, unix.IPPROTO_IP)
	if err != nil {
	...
	if err != nil {
		if err == syscall.EWOULDBLOCK {
			panic("It is not ready to be read")
		}
		fmt.Printf("Recvfrom: %#v", err)
		unix.Close(serverFD)
		return
	}
	fmt.Printf("< %s\n", string(response))
