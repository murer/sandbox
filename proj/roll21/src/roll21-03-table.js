(function($, $d) {
    ...
    function addRow(id, req, resp) {
        var rt = $d.find('#' + id)
        if(!rt.length) {
            console.log('aa', $d.find('#wrapper'))
            $d.find('#wrapper').append('<div id="' + id + '"><h3>' + id + '</h3><table><thead><tr><th>roll</th><th>action</th></tr></thead></table></div>')
            $d.find('table').attr('border', '1')
            rt = $d.find('#' + id)
            req.rolls.forEach(v => {
                console.log('v', v)
