...
# fixing .deb
dpkg -x ../oracle.deb oraextract
dpkg -e ../oracle.deb oraextract/DEBIAN
cat oraextract/DEBIAN/control | sed "s/^Depends:.*/Depends: libaio (>= 0.3.96) | libaio1 (>= 0.3.96)/g" > control.fix
mv control.fix oraextract/DEBIAN/control
dpkg --build oraextract
ls -lart

# install
sudo apt-get -y install libc6-i386 bc
sudo dpkg -i --force-architecture ../libaio.deb
